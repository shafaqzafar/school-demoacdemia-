...
export default function PDFs(){
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const res = await sharedContentApi.list({ type: 'pdf' });
  ...
  const subjects = useMemo(() => Array.from(new Set((rows || []).map((r) => r?.subjectName || '-'))).filter(Boolean), [rows]);
  ...
  const kpis = useMemo(()=>({ total: demoPDFs.length, subjects: new Set(demoPDFs.map(n=>n.subject)).size, recent: demoPDFs.filter(n => (Date.now()-n.date.getTime())/(1000*60*60*24) <= 7).length, pages: 0}), [demoPDFs]);
  ...
  return (
    <Box pt={{ base:'130px', md:'80px', xl:'80px' }}>
      ...
      {loading ? (
        <Card p='16px' mb='16px'>
          <Text color={textSecondary}>Loading PDFs...</Text>
        </Card>
      ) : null}
