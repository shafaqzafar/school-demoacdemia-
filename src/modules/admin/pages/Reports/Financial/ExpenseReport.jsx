...
export default function ExpenseReport() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const result = await reportsApi.financial.expense({ ...filters, campusId });
            setData(result);
        } catch (error) { console.error(error); } finally { setLoading(false); }
    };

    const totalExpense = data.reduce((acc, curr) => acc + Number(curr.amount || 0), 0);
            ...
            <Flex direction='column'>
                ...
                <SimpleGrid columns={{ base: 1, md: 3 }} gap='20px' mb='20px'>
                    <Card p='20px'>
                        <Stat>
                            ...
                            <StatNumber>${totalExpense.toLocaleString()}</StatNumber>
                ...
                <Card p='20px'>
                    <Table variant='simple'>
                        <Thead>
                            <Tr><Th>Date</Th><Th>Category</Th><Th>Description</Th><Th>Payee</Th><Th>Amount</Th></Tr>
                        </Thead>
                        <Tbody>
