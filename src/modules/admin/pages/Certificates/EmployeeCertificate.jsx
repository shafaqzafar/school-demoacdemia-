...
export default function EmployeeCertificate() {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await employeeCertificateApi.delete(id);
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            await employeeCertificateApi.update(active.id, payload);
    ...
    const buildPrintableHtml = ({ cert, employee, template }) => {
        ...
        const placeholders = {
            name: employee?.name || cert?.personName || 'Employee',
...
<body>
  <div class="page">
    <div class="card">
      ...
      <div class="content">
        ...
        <div class="meta">
          <div>Employee: <strong>${escapeHtml(placeholders.name)}</strong></div>
          <div>${escapeHtml(placeholders.designation ? `Designation: ${placeholders.designation}` : '')}</div>
        </div>
  ...
  <script>
    ...
    const printCertificate = (cert) => {
        const employee = employees.find((e) => String(e.id) === String(cert.employeeId));
        ...
        const html = buildPrintableHtml({ cert, employee, template });
    ...
    const filteredCertificates = certificates.filter((c) => {
        ...
        if (search) {
            ...
            const name = emp?.name || c.personName || '';
    ...
    const exportReportPdf = () => {
        ...
        doc.setFontSize(14);
        doc.text('Employee Certificates Report', margin, 52);
        ...
        for (const c of rows) {
            ...
            const cells = [
                String(c.id ?? ''),
                String(emp?.name || c.personName || 'Employee').slice(0, 28),
        ...
        doc.save(`Employee_Certificates_Report_${now.toISOString().slice(0, 10)}.pdf`);
    };
            ...
            <Card p={4} mb={5}>
                <Flex gap={3} direction={{ base: 'column', md: 'row' }} align={{ md: 'center' }}>
                    <InputGroup maxW='320px'>
                        ...
                        <Input placeholder='Search employee/template' value={search} onChange={(e) => setSearch(e.target.value)} />
                    </InputGroup>
                    ...
                    <ModalBody>
                        {active ? (
                            <Box>
                                ...
                                <Text><strong>Employee:</strong> {employees.find((e) => String(e.id) === String(active.employeeId))?.name || active.personName || 'Employee'}</Text>
